...
func _fixed_process(delta):
	...
	if (low + position.y > root.get_node("score/left_score").get_texture().get_size().y and Input.is_action_pressed("right_move_up")):
		velocity.y = -speed * delta
	if (high + position.y < get_viewport_rect().size.y and Input.is_action_pressed("right_move_down")):
		velocity.y = speed * delta
...
func powerup(var type):
	if (type == ARMOR):
		root.get_node("player2_powerups/armor/overlay").set_time(POWERUP_DURATION)
		for block in blocks:
			block.armor_up()
			armor_timer = POWERUP_DURATION
	elif (type == CURVE_DOWN):
		root.get_node("player2_powerups/curve/overlay").set_time(POWERUP_DURATION)
		root.get_node("player2_powerups/curve").set_frame(type)
		curve_amount *= 0.66
		...
		root.get_node("player2_powerups/pad_speed/overlay").set_time(POWERUP_DURATION)
		root.get_node("player2_powerups/pad_speed").set_frame(type)
		speed -= 200
		...
		root.get_node("player2_powerups/ball_speed/overlay").set_time(POWERUP_DURATION)
		root.get_node("player2_powerups/ball_speed").set_frame(type)
